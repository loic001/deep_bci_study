...
for item in plot_dict:
    ...
    for expe in ['meeg48_transfer']:
        grad_target = [rec['smooth_grad_target'].mean(axis=0) for rec in res if (rec['model'] == model and rec['expe'] == expe)]
        ...
        to_plot.append({
            ...
            'grad_non_target': grad_non_target,
...
figs = []
for item in to_plot:
    grad_target = item.get('grad_target')
    for subject_data in grad_target:
        expe = item.get('expe')
        model = item.get('model')
        evoked_target = mne.EvokedArray(subject_data, raw_vpfat.info)
        scalings = {'eeg': 10**9} if expe == 'rsvp48' else {'eeg': 10**7}
        titles = {'eeg': '{}_{}'.format(expe, model)}
        fig = evoked_target.plot_image(units={'eeg': 'Gradient'}, titles=titles, scalings=scalings, show=False)
        figs.append(fig)
    break
...
multipage('viz_ind_meeg48_transfer.pdf', figs)
